...
Repository: `cumt-campusnet-auto-login`

一个面向中国矿业大学校园网认证页面的非官方自动登录脚本项目。  
项目目标是将手动登录校园网认证页面的过程整理为一个可复用、可审查、可本地配置的 Shell 脚本。
...
```bash
git clone https://github.com/SundeqiX1/cumt-campusnet-auto-login.git
...
公开仓库保留默认门户参数，可以减少重复适配成本；真正敏感的内容是账号、密码、Cookie、token、日志等个人数据，因此配置模板中必须留空。
...
从功能边界看，本项目只尝试把校园网认证页面上的常规登录动作本地脚本化：

- 调用的是学校现有认证入口
- 不提供绕过认证、破解口令、爆破、扫描或批量代登能力
- 依赖账号持有人自行提供本地配置，不内置任何真实凭据

这类脚本不应被理解为破解、绕过或替代官方认证系统；同时，它也不是零风险工具。主要风险包括：
...
在本仓库整理阶段，公开可检索的中国矿业大学信息化页面中未检索到“明确禁止脚本化调用校园网登录页”的公开条款；但这不代表校方对本项目或此类使用方式作出正式许可。学校公告、网络管理要求或服务条款如有更新，应以最新官方规则为准。
...
本仓库只适合公开脚本代码、配置模板和说明文档。以下内容不适合公开发布：
...
公开发布前，应确认仓库中不包含真实账号密码、Cookie、私有日志和个人配置。
